...
from dbtbt.utils.utils import read_yaml, HEADER
...
def get_parsed_args():
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="List of models to run, e.g. dim_my_dim_table fct_my_fact_table",
    )
    parser.add_argument(
        "-e",
        "--exclude",
        nargs="+",
        help="List of models to exclude, e.g. dim_my_dim_table fct_my_fact_table",
    )
    parser.add_argument(
        "-t",
        "--target",
    ...
    parser.add_argument(
        ...
        "--wap",
        action="store_true",
        help="If set, will deploy using Write-Audit-Publish approach, requires a `*_audit` target",
    )
    parser.add_argument(
        "-r",
        "--rebuild",
        action="store_true",
        help="If set, will rebuild prod manifest",
    )
    parser.add_argument(
        "-i",
        "--incremental",
        action="store_true",
        help="If set, will also run models incrementally",
    )
    parser.add_argument(
        "-d",
        "--dry-run",
        action="store_true",
        help="If set, will print commands to screen only",
    )
    parser.add_argument(
        "-p",
        "--print-dry-run",
        action="store_true",
        help="If set, will print commands to screen only including the models affected by --models",
    )
    parser.add_argument(
        "-u",
        "--upstream-source",
...
def main():
    ...
    assert args.action in actions, f"'{args.action}' is currently not supported"

    config = read_yaml(args.config_file)

    if "refresh_deps" not in config:
        config["refresh_deps"] = True
