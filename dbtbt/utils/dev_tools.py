...
def read_yaml(yaml_path):
    ...
    try:
        ...
        logger.info(color_me(f"Could not find '{yaml_path}'. Please check that '{yaml_path}' exists.", "red"))
        raise
...
def print_dbt_model_dependencies(model_list, args):
    if args.dry_run and args.print_dry_run:
        print_msg(f"The following models would be affected by your model selection:", "-")
